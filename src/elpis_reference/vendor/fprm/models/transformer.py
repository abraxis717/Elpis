...
class FixedPointTransformer(nn.Module):
    ...
    def forward(
        ...
        collect_layers: bool = False, route_fn=None, **kwargs
    ) -> Tuple[torch.Tensor, Tuple[List[torch.Tensor], List[torch.Tensor]]]:
        # route_fn(layer_idx, layer_states_so_far) -> routed input for that layer
        # (cross-grid trajectory routing); requires collect_layers=True.
        ...
        for i, layer in enumerate(self.layers):
            if route_fn is not None and i > 0:
                hidden_states = route_fn(i, layer_states)
            hidden_states = layer(
